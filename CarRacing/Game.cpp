...
void Object::update_pos() { pos_y += relative_speed; }
int Object::right_border() { return pos_x + width - 1; }
int Object::left_border() { return pos_x; }
int Object::front_border() { return pos_y; }
int Object::back_border() { return pos_y + height - 1; }
int Object::get_pos_x() { return pos_x; }
int Object::get_pos_y() { return pos_y; }
int Object::get_relative_speed() { return relative_speed; }
void Object::set_relative_speed(int new_speed) { relative_speed = new_speed; }
Player::Player(int pos_x, int pos_y, int relative_speed)
    : Object(pos_x, pos_y, relative_speed) {}
Player::~Player() {}
void Player::set_pos_x(int new_pos) { pos_x = new_pos; }
const SpriteRepository::Sprite& Player::get_sprite() {
    return SpriteRepository::get_player();
}

Car::Car(int pos_x, int pos_y, int relative_speed)
    : Object(pos_x, pos_y, relative_speed) {}
Car::~Car() {}
...
Truck::Truck(int pos_x, int pos_y, int relative_speed)
    : Object(pos_x, pos_y, relative_speed) {}
Truck::~Truck() {}
...
Road::Road()
    : player(2 * SpriteRepository::width, height - SpriteRepository::height),
      engine(),
      objects(),
      dist(0) {
    clear();
...
std::vector<int> Road::free_pos() {
    ...
    for (int i = 0; i < first_row.size(); ++i) {
        bool free = true;
        for (int j = i; j < i + SpriteRepository::width && j < first_row.size();
             ++j) {
            if (first_row[j] != '.') {
                free = false;
                break;
...
bool Road::update() {
    ...
    for (auto it = objects.begin(); it != objects.end();) {
        ...
        for (auto& another : objects) {
            ...
            int dist = std::min(another->right_border(), obj->right_border()) -
                       std::max(another->left_border(), obj->left_border());
            if (another->back_border() + 1 == obj->front_border() &&
                another->get_relative_speed() > obj->get_relative_speed() &&
                dist >= 0) {
...
void Road::render_objs() {
    for (auto& obj : objects) {
        ...
        for (int y = std::max(obj->get_pos_y(), 0);
             y < std::min(height, obj->get_pos_y() + obj->height); ++y) {
            for (int x = obj->get_pos_x();
                 x < std::min(width, obj->get_pos_x() + obj->width); ++x) {
...
bool Road::render_player() {
    ...
    for (int y = player.get_pos_y();
         y < std::min(height, player.get_pos_y() + player.height); ++y) {
        for (int x = player.get_pos_x();
             x < std::min(width, player.get_pos_x() + player.width); ++x) {
            if (road[y][x] != '.' &&
                sprite[y - player.get_pos_y()][x - player.get_pos_x()] != '.') {
                return true;
            }
...
void Road::render_broken_player() {
    const SpriteRepository::Sprite& sprite =
        SpriteRepository::get_broken_player();
    for (int y = player.get_pos_y();
         y < std::min(height, player.get_pos_y() + player.height); ++y) {
        for (int x = player.get_pos_x();
             x < std::min(width, player.get_pos_x() + player.width); ++x) {
...
int Road::get_score() { return dist; }

void Road::set_max_dist(int new_max_dist) { max_dist = new_max_dist; }
...
void CarRacing::get_player_commands() {
    if (engine.key_pressed('A')) {
        ...
        road.player.set_pos_x(std::min(road.width - road.player.width,
                                       road.player.get_pos_x() + 1));
    }
}
